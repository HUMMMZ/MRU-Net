...
class ConfusionMatrix(object):
    ...
    def compute(self):
        h = self.mat.float()
        dice = (2 * torch.diag(h)[1]) / (h.sum(0)[1] + h.sum(1)[1])  # Dice=2TP/(FP+2TP+FN)
        # 计算全局预测准确率(混淆矩阵的对角线为预测正确的个数)
        acc_global = torch.diag(h).sum() / h.sum()  # PA=(TP+TN)/(TP+TN+FP+FN)
        Jaccard = torch.diag(h)[1] / (h.sum()-torch.diag(h)[0])
        # 计算每个类别的准确率
        acc = torch.diag(h) / h.sum(1)  # Recall=TP/(TP+FN)  TN/(FP+TN)
        recall = torch.diag(h) / h.sum(0)
        # precision = torch.diag(h)[1] / h.sum(1)[1]  # Precision=TP/(TP+FP)
        FPR = h[:, 0][1] / h.sum(0)[0]
        FNR = h[:, -1][0] / h.sum(0)[1]

        # 计算每个类别预测与真实目标的iou
        iu = torch.diag(h) / (h.sum(1) + h.sum(0) - torch.diag(h))  # TP/(TP+FN+FP)   TN/(FP+FN+TN)
        return acc_global, acc, iu, recall, FPR, dice, FNR, Jaccard
    ...
    def __str__(self):
        acc_global, acc, iu, recall, FPR, dice, FNR, Jaccard = self.compute()
        return (
            'Dice: {:.4f}\n'
            'PA: {:.2f}\n'
            'Jaccard: {:.2f}\n'
...
class DiceCoefficient(object):
    ...
    def update(self, pred, target):
        ...
        self.cumulative_dice += multiclass_dice_coeff(pred[:, 1:], dice_target[0][:, 1:], ignore_index=self.ignore_index)
        self.count += 1
...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, header=None):
        ...
        if torch.cuda.is_available():  # GPU训练输出参数数据
